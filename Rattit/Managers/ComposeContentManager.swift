...
class ComposeContentManager {
    ...
    func postNewQuestion(title: String, words: String, completion: @escaping () -> Void, errorHandler: @escaping () -> Void) {
        print("postNewQuestion, title=\(title), words=\(words)")
        
        var newQuestionDic = [String: Any]()
        newQuestionDic["title"] = title as Any
        newQuestionDic["words"] = words as Any
        newQuestionDic["access_level"] = self.shareToLevel.rawValue as Any
        newQuestionDic["hash_tags"] = ["iPhoneTesting"] as Any
        
        if self.pickedPlaceFromGoogle != nil {
            newQuestionDic["google_place"] = self.pickedPlaceFromGoogle!.wrapIntoLocationPostObj() as Any
        }
        
        
        if self.indexOfCheckedPhotos.count > 0 {
            self.publicUrlsOfCheckedPhotos.removeAll()
            let numberOfCheckedPhotos = self.indexOfCheckedPhotos.count
            let stubPublicUrlObj: [String: Any] = ["image_url": "",
                                                   "height": 500,
            ...
            self.indexOfCheckedPhotos.enumerated().forEach({ (offset, indexVal) in
                ...
                ComposeContentManager.uploadOneImageToServer(rawImage: imageChosen, gotImageUrl: { (imageUrl) in
                    ...
                    Network.sharedInstance.callRattitContentService(httpRequest: .postNewQuestionContent(bodyDic: newQuestionDic), completion: { (dataValue) in
                            ...
                            print("publishing new Question got failure: \(error.localizedDescription)")
                            DispatchQueue.main.async {
                                errorHandler()
                            }
                        })
